...
class WorkerPool:
    ...
    async def start(self, account_id: str) -> Worker:
        ...
        if worker and worker.running:
            logger.info("Worker already running for account_id=%s", account_id)
            return worker
        worker = worker or self._worker_factory(account_id)
        self._workers[account_id] = worker
        worker.start()
        logger.info("Worker started for account_id=%s", account_id)
        return worker
    ...
    async def stop(self, account_id: str) -> None:
        ...
        if worker:
            await worker.stop()
            logger.info("Worker stopped for account_id=%s", account_id)
